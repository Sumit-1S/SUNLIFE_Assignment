...
create table USER_TABLE(ID INT auto_increment PRIMARY KEY, USER_NAME VARCHAR(20),
						AGE INT, GENDER CHAR(1), SALARY DECIMAL(10,5));
                        
INSERT INTO USER_TABLE VALUES(001,'SUMIT', 23,'M',2000);
INSERT INTO USER_TABLE VALUES(002,'SUMITB', 24,'M',3000);
INSERT INTO USER_TABLE VALUES(003,'SUMITC', 25,'M',4000);
INSERT INTO USER_TABLE VALUES(004,'SUMITD', 26,'M',5000);
...
select * from customers where not ( country='Germany' or city='Berlin' );

select * from customers where city like '_d';

select * from customers where (customername is not null and city is not null);

select * from customers where country='Germany' and (city='Mannheim' or city='Berlin');

select * from customers where not(country='Germany' or country='USA');

update customers set customerName = 'AlphaOne' where (country='Germany' and city='Mannheim');

select city,count(city) from customers group by city ;

select * from products where productname like 'c%' and (price>18 and price<25);

select categoryid,count(categoryid) from products group by categoryid order by categoryid desc;

select categoryid, price from products order by categoryid desc, price asc;
...
select city, country, shipperid from customers c, orders o where c.customerid = o.customerid;

select * from products p, categories c where p.categoryid = c.categoryid;

select customername,postalcode,city,orderdate,quantity from customers c, orders o, order_details od where c.customerid = o.customerid and o.orderid=od.orderid;

select productname, unit,price, orderdate from products p, order_details od, orders o where p.productid = od.productid and od.orderid = o.orderid;
